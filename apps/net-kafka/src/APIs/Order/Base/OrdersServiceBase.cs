...
public abstract class OrdersServiceBase : IOrdersService
{
    ...
    public OrdersServiceBase(NetKafkaDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<Order> CreateOrder(OrderCreateInput createDto)
    {
        ...
        if (createDto.UpcomingCustomer != null)
        {
            order.UpcomingCustomer = await _context
                .Customers.Where(customer => createDto.UpcomingCustomer.Id == customer.Id)
                .FirstOrDefaultAsync();
        }
    ...
    public async Task UpdateOrder(OrderWhereUniqueInput uniqueId, OrderUpdateInput updateDto)
    {
        ...
        {
            order.UpcomingCustomer = await _context
                .Customers.Where(customer => updateDto.UpcomingCustomer == customer.Id)
                .FirstOrDefaultAsync();
        }
    ...
    /// </summary>
    public async Task<Customer> GetUpcomingCustomer(OrderWhereUniqueInput uniqueId)
    {
        var order = await _context
            .Orders.Where(order => order.Id == uniqueId.Id)
            .Include(order => order.UpcomingCustomer)
            .FirstOrDefaultAsync();
        if (order == null)
        {
            throw new NotFoundException();
        }
        return order.UpcomingCustomer.ToDto();
    }
}
